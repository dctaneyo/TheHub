...
function makeRequest(body: unknown): Request {
  return new Request("http://localhost/api/tasks/bulk", { method: "POST", body: JSON.stringify(body) });
}

const arlSession = { id: "arl-1", tenantId: "tenant-a", userType: "arl" as const };
...
describe("POST /api/tasks/bulk", () => {
  ...
  it("returns 403 when the caller lacks TASKS_EDIT", async () => {
  ...
  it("refreshes task timers and logs the bulk operation with this tenant's id after a successful action", async () => {
    await POST(makeRequest({ action: "clear-completions-today" }));

    expect(mockRefreshTaskTimers).toHaveBeenCalled();
    expect(mockLogBulkOperation).toHaveBeenCalledWith(expect.objectContaining({
      tenantId: "tenant-a", userId: "arl-1", operation: "clear-completions-today", entityType: "tasks",
    }));
  });
