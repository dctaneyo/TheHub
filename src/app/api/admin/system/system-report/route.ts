...
// Moved from /api/data-management/system-report — whole-database/process
...
export async function GET() {
  ...
  try {
    ...
    const tableInfo = sqlite.prepare("SELECT name FROM sqlite_master WHERE type='table' ORDER BY name").all() as { name: string }[];
    ...
    return apiSuccess({
      ...
      database: { path: dbPath, size: dbSize, sizeFormatted: formatBytes(dbSize), tables: tableSizes },
      counts: {
        locations, arls, tasks, taskCompletions, messages, messageReads, messageReactions,
        conversations, sessions, onlineSessions, forms, broadcasts, broadcastMessages,
        broadcastQuestions, emergencyMessages, notifications,
      },
      system: {
