        ...
        {virtualizer.getVirtualItems().map((vRow) => {
          ...
          return (
            ...
            >
              <div
                className={cn(
                  "group relative border-l-4 border-b border-border px-4 py-3 transition-colors hover:bg-muted/50 active:bg-muted/50",
              ...
              >
                <div className="flex items-start gap-3">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      {notification.actionUrl && (
                        <a
                          ...
                          className="text-xs font-semibold text-[var(--hub-red)] hover:underline active:underline"
                        >
                  ...
                  <button
                    ...
                    className="flex shrink-0 h-6 w-6 items-center justify-center rounded-md text-muted-foreground hover:bg-muted hover:text-foreground active:bg-muted active:text-foreground transition-colors"
                    title="Dismiss"
                  >
...
export function NotificationPanel({ open, onClose, onCountsUpdate, taskNotifications = [], onDismissTask, onDismissAllTasks, isMirroring = false, targetLocationId }: NotificationPanelProps) {
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/notifications?limit=20${unreadParam}${locParam}`);
      if (res.ok) {
  ...
  }, [filter, onCountsUpdate, isMirroring, targetLocationId]);
        ...
        <div className="flex items-center justify-between border-b border-border px-4 py-3">
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className="flex h-8 items-center gap-1 rounded-lg px-2 text-xs font-semibold text-muted-foreground hover:bg-muted hover:text-foreground active:bg-muted active:text-foreground transition-colors"
              title="Dismiss all"
            >
          ...
          <button
            ...
            className="flex h-8 w-8 items-center justify-center rounded-lg text-muted-foreground hover:bg-muted hover:text-foreground active:bg-muted active:text-foreground transition-colors"
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    {onDismissTask && (
                      <button onClick={() => onDismissTask(tn.id)}
                        className="shrink-0 text-muted-foreground/40 hover:text-muted-foreground active:text-muted-foreground transition-colors">
                        <X className="h-3.5 w-3.5" />
                      </button>
                    )}
                  </div>
