...
const buttonVariants = cva(
  ...
  {
    variants: {
      // Every hover: is paired with an active: of the same treatment — this
      // is a touchscreen kiosk product too (no hover on tap), and DESIGN.md
      // §6 flags hover-only feedback as invisible on that hardware. Button
      // is shared by both the kiosk/ARL surfaces and the mouse-driven Admin
      // Console, so hover stays for desktop pointer users; active just
      // mirrors it rather than replacing it, so touch users see the same
      // feedback hover would have given them.
      variant: {
        ...
        outline:
          "border bg-background hover:bg-accent hover:text-accent-foreground active:bg-accent active:text-accent-foreground dark:bg-input/30 dark:border-input dark:hover:bg-input/50 dark:active:bg-input/50",
        secondary:
