...
/*
 * NIGHTSHIFT — Dark HUD / Mission Control
 *
 * Vertical 3-zone layout:
 *   Zone 1 (top):    Command bar with arc gauges + neon status chips
 *   Zone 2 (center): Wide full-width timeline (no sidebars)
 *   Zone 3 (bottom): Horizontal dock with 4 equal panels side by side
 *                     [Completed] [Missed] [Upcoming] [Leaderboard]
 *
 * Structurally: stacked vertically, NOT columns.
 */
...
function ArcGauge({ value, max, color, size = 80 }: { value: number; max: number; color: string; size?: number }) {
  ...
  return (
    <svg width={size} height={size / 2 + 8} viewBox={`0 0 ${size} ${size / 2 + 8}`} className="overflow-visible">
      <defs>
        <filter id={id}><feGaussianBlur stdDeviation="2.5" result="b" /><feComposite in="SourceGraphic" in2="b" operator="over" /></filter>
      </defs>
      <path d={`M 5 ${size / 2 + 3} A ${r} ${r} 0 0 1 ${size - 5} ${size / 2 + 3}`} fill="none" stroke="rgba(255,255,255,0.06)" strokeWidth="5" strokeLinecap="round" />
      <motion.path d={`M 5 ${size / 2 + 3} A ${r} ${r} 0 0 1 ${size - 5} ${size / 2 + 3}`} fill="none" stroke={color} strokeWidth="5" strokeLinecap="round" filter={`url(#${id})`}
...
export function NightshiftLayout({
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-[#0a0e1a] text-white relative">
          ...
          <div className="flex items-end gap-4">
            <div className="flex flex-col items-center">
              ...
              <div className="text-center -mt-0.5">
                <p className="text-lg font-black font-mono tabular-nums text-cyan-400" style={{ textShadow: "0 0 12px rgba(6,182,212,0.4)" }}>{pct}%</p>
      ...
      {/* ═══ ZONE 3: Bottom Dock — 4 horizontal panels ═══ */}
      <div className="shrink-0 h-[200px] border-t border-cyan-900/20 bg-[#0b0f1c]/60 flex gap-px relative z-20">
        {/* Completed */}
        <div className="flex-1 flex flex-col overflow-hidden border-r border-slate-800/30">
          <div className="shrink-0 flex items-center gap-2 px-3 pt-2 pb-1">
            ...
            <span className="text-[8px] font-mono font-bold uppercase tracking-[0.15em] text-emerald-500/60">Completed</span>
            <span className="ml-auto text-[8px] font-mono text-slate-600">{completedTasks.length}</span>
          </div>
          <div className="flex-1 overflow-y-auto px-2 pb-2 space-y-0.5">
            {completedTasks.length === 0 ? (
              <p className="text-[8px] font-mono text-slate-700 text-center py-2">—</p>
            ) : completedTasks.map((t) => (
              <div key={t.id} className="flex items-center gap-1.5 rounded bg-emerald-950/10 border border-emerald-900/20 px-2 py-0.5">
                <CheckCircle2 className="h-2.5 w-2.5 text-emerald-600 shrink-0" />
                <span className="flex-1 text-[8px] font-mono text-slate-600 line-through truncate">{t.title}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col overflow-hidden border-r border-slate-800/30">
          <div className="shrink-0 flex items-center gap-2 px-3 pt-2 pb-1">
            ...
            <span className="text-[8px] font-mono font-bold uppercase tracking-[0.15em] text-red-500/60">Missed</span>
            <span className="ml-auto text-[8px] font-mono text-slate-600">{missedYesterday.length}</span>
          </div>
          <div className="flex-1 overflow-y-auto px-2 pb-2 space-y-0.5">
            {missedYesterday.length === 0 ? (
              <p className="text-[8px] font-mono text-emerald-700 text-center py-2">None</p>
            ) : missedYesterday.map((t) => (
              <div key={t.id} className="flex items-center gap-1.5 rounded bg-red-950/10 border border-red-900/20 px-2 py-0.5">
                <XCircle className="h-2.5 w-2.5 text-red-700 shrink-0" />
                <span className="flex-1 text-[8px] font-mono text-slate-600 truncate">{t.title}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col overflow-hidden border-r border-slate-800/30">
          <div className="shrink-0 flex items-center gap-2 px-3 pt-2 pb-1">
            <Zap className="h-2.5 w-2.5 text-cyan-400" style={{ filter: "drop-shadow(0 0 4px rgba(6,182,212,0.5))" }} />
            <span className="text-[8px] font-mono font-bold uppercase tracking-[0.15em] text-cyan-500/60">Upcoming</span>
          </div>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          <div className="shrink-0 flex items-center gap-2 px-3 pt-2 pb-1">
            <Trophy className="h-2.5 w-2.5 text-amber-400" style={{ filter: "drop-shadow(0 0 4px rgba(245,158,11,0.4))" }} />
            <span className="text-[8px] font-mono font-bold uppercase tracking-[0.15em] text-amber-500/60">Rankings</span>
          </div>
          <div className="flex-1 overflow-y-auto px-2 pb-2">
